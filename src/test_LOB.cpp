...
void test_order() {
  /* 測試下單與撮合 */
  {
    ...
    cout << "place order and match order passed!\n";
  }
...
void test_query() {
  ...
  /* 測試 get_best_ask_price 和 get_best_ask_volume */
  {
    ...
    cout << "get_best_ask_price passed!\n";
  }
  ...
  /* 測試當 k < 掛單數時的 get_top_k_info */
  {
    ...
    vector<PriceLevelInfo> lob_top_k = lob.get_top_k_info(test_num, Side::BUY);
    for (int i = 0; i < info_list.size(); i++) {
      assert(info_list[i].price == lob_top_k[i].price);
      assert(info_list[i].volume == lob_top_k[i].volume);
    }
    ...
    /* 測試當 k < 掛單數時的 get_top_k_info */
    {
      ...
      vector<PriceLevelInfo> lob_top_k =
          lob.get_top_k_info(test_num, Side::BUY);
      for (int i = 0; i < info_list.size(); i++) {
        assert(info_list[i].price == lob_top_k[i].price);
        assert(info_list[i].volume == lob_top_k[i].volume);
      }
      cout << "get_top_k_info passed!\n";
    }
  }
